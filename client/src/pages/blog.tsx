  ...
  return (
    <div className="bg-white min-h-screen">
      <SEO
        ...
        canonical="https://infiniterankers.io/blog"
      />
      ...
      <section className="py-8 sm:py-10 lg:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              {blogPosts.filter(p => p.featured).map((post, i) => (
                <FadeIn key={post.title} delay={i * 0.1}>
                  <div className="glass-card rounded-xl overflow-hidden group cursor-pointer h-full transition-all duration-300 hover:border-brand-blue/10">
                    <div className="h-40 sm:h-48 lg:h-52 bg-gradient-to-br from-brand-blue/5 via-brand-violet/5 to-brand-blue-light/5 relative overflow-hidden">
          ...
          <div className="space-y-3 sm:space-y-4">
            {blogPosts.filter(p => !p.featured).map((post, i) => (
              <FadeIn key={post.title} delay={i * 0.05}>
                <div className="glass-card glass-card-hover rounded-xl p-4 sm:p-5 lg:p-6 group cursor-pointer transition-all duration-300">
                  <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
                    ...
                    <div className="flex-shrink-0 self-center">
                      <span className="text-sm text-brand-blue font-medium flex items-center gap-1 min-h-[44px] sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
          ...
          <div className="text-center mt-8 sm:mt-12">
            <a href="https://infiniterankers.io" target="_blank" rel="noopener noreferrer" className="text-sm text-brand-muted hover:text-brand-blue transition-colors min-h-[44px] inline-flex items-center" data-testid="link-blog-site">Read more at infiniterankers.io →</a>
          </div>
        </div>
      </section>
    </div>
